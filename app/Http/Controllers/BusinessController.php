...
class BusinessController extends Controller
{
    ...
    {
        $signedAgreementId = $request->customer_id ?? $request->signed_agreement_id ?? Str::uuid();
    ...
    public function startBusinessVerification(Request $request)
    {
        ...
        BusinessCustomer::firstOrCreate([
            'session_id'          => session()->get('customer_id') ?? $signedAgreementId,
            'type'                => 'business',
            'signed_agreement_id' => session('customer_submission_id'),
            'user_agent'          => $request->userAgent(),
            'ip_address'          => $request->ip(),
            'customer_id'         => session('customer_submission_id') ?? session('customer_id') ?? $signedAgreementId,
        ]);
