...
interface Props {
  ...
  accountPurposes: readonly AccountPurpose[]; // updated type
...
export default function FinancialInformation({
  ...
  const sourceOfFunds = [
    { code: "CompanyFunds", label: "Company Funds" },
    { code: "EcommerceReseller", label: "Ecommerce Reseller" },
    { code: "GamblingProceeds", label: "Gambling Proceeds" },
    { code: "Gifts", label: "Gifts" },
    { code: "GovernmentBenefits", label: "Government Benefits" },
    { code: "Inheritance", label: "Inheritance" },
    { code: "InvestmentsLoans", label: "Investments Loans" },
    { code: "PensionRetirement", label: "Pension Retirement" },
    { code: "Salary", label: "Salary" },
    { code: "SaleOfAssetsRealEstate", label: "Sale Of Assets or Real Estate" },
    { code: "Savings", label: "Savings" },
    { code: "SomeoneElsesFunds", label: "Someone Else's Funds" },
  ] as const;
  ...
  const handleCheckbox = (activity: string) => {
    ...
    if (activity === 'none_of_the_above') {
      nextActs = ['none_of_the_above'];
    } else {
      nextActs = nextActs.filter((a) => a !== 'none_of_the_above');
      if (nextActs.includes(activity)) {
        nextActs = nextActs.filter((a) => a !== activity);
      } else {
        nextActs.push(activity);
      }
      if (nextActs.length === 0) nextActs = ['none_of_the_above'];
    }
  ...
  return (
    <div className="dark:bg-gray-800 bg-white shadow-xl sm:rounded-lg p-6">
      ...
      <div className="grid grid-cols-1 gap-y-6 gap-x-4 sm:grid-cols-2">
        ...
        {local.account_purpose === 'Other' && (
          <div>
            ...
            {errors.account_purpose_other && (
              <p className="text-sm text-red-600">{errors.account_purpose_other}</p>
            )}
          </div>
        )}
            ...
            {[
              ...
              <label key={act} className="flex items-center">
                <input
                  ...
                  disabled={act === 'none_of_the_above' && local.high_risk_activities.length > 1}
        ...
        {!local.high_risk_activities.includes('none_of_the_above') && (
          <div className="sm:col-span-2">
